...
struct SettingsView: View {
    ...
    @State private var showResetGuideConfirmation = false
            ...
            ZStack {
                Form {
                    Section("요금 설정") {
                        ...
                        .coachMarkTarget(id: "regionSetting")
                    }
                    ...
                    .coachMarkTarget(id: "surchargeMode")
                    ...
                    .coachMarkTarget(id: "soundSetting")

                    Section("도움말") {
                        Button(action: {
                            coachMarkManager.resetAllCoachMarks()
                            showResetGuideConfirmation = true
                        }) {
                            HStack {
                                Label("가이드 다시 보기", systemImage: "questionmark.circle")
                                Spacer()
                            }
                        }
                        .coachMarkTarget(id: "resetGuide")
                    }
                ...
                if coachMarkManager.isShowingCoachMark,
                   coachMarkManager.currentScreenId == "settings",
            ...
            .onAppear {
                ...
                if coachMarkManager.shouldShowCoachMarks(for: "settings") {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        coachMarkManager.startCoachMarks(for: "settings")
                    }
                }
            }
            .alert("가이드 초기화", isPresented: $showResetGuideConfirmation) {
                Button("확인", role: .cancel) { }
            } message: {
                Text("각 화면 방문 시 가이드가 다시 표시됩니다.")
            }
        }
    }
